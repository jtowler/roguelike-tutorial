...
class BaseEventHandler(tcod.event.EventDispatch[ActionOrHandler]):
    def handle_events(self, event: tcod.event.Event) -> BaseEventHandler:
        """Handle an event or return the next active event handler"""
...
class PopupMessage(BaseEventHandler):
    ...
    def on_render(self, console: tcod.console.Console) -> None:
        """Render the parent and dim the results, then print the message on top"""
...
class EventHandler(BaseEventHandler):
    ...
    def handle_action(self, action: Optional[Action]) -> bool:
        """
        Handle actions return from the event methods.
...
class GameOverEventHandler(EventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[BaseEventHandler]:
        ...
        if event.sym == tcod.event.K_r:
            ...
            return MainGameEventHandler(new_game(self.engine.debug))
...
class HistoryViewer(EventHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        ...
        self.engine.message_log.render_messages(log_console,
                                                1,
                                                1,
                                                log_console.width - 2,
...
class InventoryEventHandler(AskUserEventHandler):
    ...
    def on_render(self, console: tcod.Console) -> None:
        """
        Render an inventory menu which displays items in the inventory and the letter to select them.
        Will move to a different position based on where the player is located so they can always
        see where they are."""
        super().on_render(console)
    ...
    def on_item_selected(self, item: Item) -> Optional[ActionOrHandler]:
        """Called whene the user selects a valid item."""
        raise NotImplementedError()
...
class AreaRangeAttackHandler(SelectIndexHandler):
    """Handles targeting an area with a given radius. Any entity within the area will be affected."""
